...
# To DO: File path change required
# mypath='D:/Work/Project/Resume Scoring/word1/' #enter your path here where you saved the resumes
mypath= cf.path
print("Mypath from top:",mypath)
# total_files = [os.path.join(mypath, f) for f in os.listdir(mypath) if os.path.isfile(os.path.join(mypath, f))]
# print("Total files from Top:",total_files)
...
def calculate_skill_score(text, skill_list):
    no_kws, not_matched_kw = check(text, skill_list)
...
#         print(substring)
        matched_kw = [ w for w in words if w.lower() in substring.lower() ]
        # not_matched_kw = [ w for w in words if w not in substring ]
        if (len(matched_kw)): 
            res_words.append(matched_kw) 
    match_kws = list(set(sum(res_words,[])))
    not_matched_kw = list(set(words)-set(match_kws))
    # print("Not matched",not_matched_kw)
    return (get_unq_skill(match_kws),not_matched_kw)
    ...
    if jd_file_path:
        ...
        try:
            for item in text_list:
                key = item.split(':')[0]
                value = item.split(':')[1:]
                value = sum([i.strip().split(',') for i in value],[])
                value = sum([i.strip().split('and') for i in value],[])
                meta_data[key] = value
        except Exception as e:
            pass
...
#         print(no_of_keyUnit,comment,total_exprience)
    ## Total score
    print("\t\n Mandatory Field Score:",score_list)
    print("\t\nComment:", comments)
    try:
